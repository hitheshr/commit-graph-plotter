import { storeLocalToken, getLocalToken } from "./getLocalToken";
import { sortCommits } from "./sortCommits";
import { storeLocalUserName } from "./getLocalToken";

export async function fetchCommits(repoOwner, repoName, token) {
    ...
    async function fetchCommitsPage(repoOwner, repoName, lastFetched) {
        ...
        if ((response.status != 200 && response.status != 201)) {
            console.log("--ERROR FETCHING GRAPHQL--"+response);
    ...
    if (await fetchCommitsPage(repoOwner, repoName, "NONE")) {
        ...
        await sortCommits(branches, heads, repoOwner, repoName);
    }
}